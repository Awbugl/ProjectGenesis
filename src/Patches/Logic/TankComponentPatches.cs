...
namespace ProjectGenesis.Patches.Logic
{
    internal static class TankComponentPatches
    {
        ...
        [HarmonyTranspiler]
        public static IEnumerable<CodeInstruction> TankComponent_GameTick_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator ilGenerator)
        {
            ...
            matcher.MatchForward(false, new CodeMatch(OpCodes.Callvirt, AccessTools.Method(typeof(CargoTraffic), nameof(CargoTraffic.TryInsertItemAtHead))));

            do
            {
                matcher.Advance(-19).InsertAndAdvance(new CodeInstruction(OpCodes.Ldarg_0),
                    new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(TankComponentPatches), nameof(TankComponent_GameTick_Insert_Method))),
                    new CodeInstruction(OpCodes.Stloc_S, local.LocalIndex));

                matcher.Advance(5).InsertAndAdvance(new CodeInstruction(OpCodes.Ldloc_S, local.LocalIndex), new CodeInstruction(OpCodes.Mul));
