...
namespace ProjectGenesis.Patches.Logic.QuantumStorage
{
    public static partial class QuantumStoragePatches
    {
        ...
        [HarmonyTranspiler]
        public static IEnumerable<CodeInstruction> FactoryStorage_GameTick_Transpiler(
        ...
        public static IEnumerable<CodeInstruction> BuildTool_Addon_Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            matcher.MatchForward(true,
                new CodeMatch(OpCodes.Ldfld, AccessTools.Field(typeof(PrefabDesc), nameof(PrefabDesc.isStorage))));
        ...
        [HarmonyPriority(Priority.VeryHigh)]
        [HarmonyPrefix]
        public static bool StorageComponent_SetEmpty(StorageComponent __instance) =>
            __instance.size != QuantumStorageSize;
            ...
            ref StorageComponent __result)
        {
            ...
            var _component = _components[orbitId - 1];
        ...
        public static bool FactoryStorage_RemoveStorageComponent(FactoryStorage __instance, int id)
        {
            ...
            var orbitId = QueryOrbitId(__instance.planet.id, id);
        ...
        [HarmonyPostfix]
        public static void FactoryStorage_TryTakeBackItems_Storage(FactoryStorage __instance, int storageId,
            ref bool __result)
        {
        ...
        [HarmonyTranspiler]
        public static IEnumerable<CodeInstruction> PlanetFactory_Mutexs_Transpiler(
            IEnumerable<CodeInstruction> instructions)
        {
            var matcher = new CodeMatcher(instructions);
            ...
            matcher.MatchForward(true,
                new CodeMatch(OpCodes.Ldfld,
                    AccessTools.Field(typeof(PlanetFactory), nameof(PlanetFactory.entityMutexs))),
                new CodeMatch(inst => inst.IsLdloc()),
                new CodeMatch(OpCodes.Ldfld,
                    AccessTools.Field(typeof(StorageComponent), nameof(StorageComponent.entityId))),
        ...
        public static IEnumerable<CodeInstruction> UIStorageWindow_Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            matcher.MatchForward(true, new CodeMatch(OpCodes.Ldloc_0),
                ...
                new CodeMatch(OpCodes.Call,
                    AccessTools.PropertyGetter(typeof(UIStorageWindow), nameof(UIStorageWindow.storageId))));
        ...
        [HarmonyTranspiler]
        public static IEnumerable<CodeInstruction> FactoryStorage_Import_Transpiler(
            IEnumerable<CodeInstruction> instructions)
        {
            var matcher = new CodeMatcher(instructions);
