...
namespace ProjectGenesis.Patches.Logic.MegaAssembler
{
    internal static partial class MegaAssemblerPatches
    {
        ...
        [HarmonyTranspiler]
        public static IEnumerable<CodeInstruction> UIBeltBuildTip_SetOutputEntity_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            matcher.InsertAndAdvance(new CodeInstruction(OpCodes.Ldloc_3), new CodeInstruction(OpCodes.Ldfld, EntityData_AssemblerId_Field),
                new CodeInstruction(OpCodes.Ldc_I4_0), new CodeInstruction(OpCodes.Ble, label), new CodeInstruction(OpCodes.Ldarg_0), new CodeInstruction(OpCodes.Ldloc_0),
                new CodeInstruction(OpCodes.Ldloc_3), new CodeInstruction(OpCodes.Ldarg_0), new CodeInstruction(OpCodes.Ldflda, UIBeltBuildTip_FilterItems_Field),
                new CodeInstruction(OpCodes.Ldarg_1), new CodeInstruction(OpCodes.Ldarg_2), new CodeInstruction(OpCodes.Call, MegaAssembler_SetOutputEntity_Patch_Method),
        ...
        [HarmonyTranspiler]
        public static IEnumerable<CodeInstruction> UISlotPicker_SetOutputEntity_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            var matcher = new CodeMatcher(instructions, generator);
            ...
            matcher.InsertAndAdvance(new CodeInstruction(OpCodes.Ldloc_1), new CodeInstruction(OpCodes.Ldfld, EntityData_AssemblerId_Field),
                new CodeInstruction(OpCodes.Ldc_I4_0), new CodeInstruction(OpCodes.Ble, label), new CodeInstruction(OpCodes.Ldarg_0), new CodeInstruction(OpCodes.Ldloc_0),
                new CodeInstruction(OpCodes.Ldloc_1), new CodeInstruction(OpCodes.Ldarg_0), new CodeInstruction(OpCodes.Ldflda, UISlotPicker_FilterItems_Field),
                new CodeInstruction(OpCodes.Ldarg_1), new CodeInstruction(OpCodes.Ldarg_2), new CodeInstruction(OpCodes.Call, MegaAssembler_SetOutputEntity_Patch_Method),
        ...
        public static int SetOutputEntity_Patch(PlanetFactory factory, EntityData entityData, ref List<int> filterItems, int entityId, int slot)
        {
        ...
        [HarmonyTranspiler]
        public static IEnumerable<CodeInstruction> UIBeltBuildTip_SetFilterToEntity_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            var matcher = new CodeMatcher(instructions, generator);

            matcher.MatchForward(true, new CodeMatch(OpCodes.Ldloc_0), new CodeMatch(OpCodes.Ldfld, PlanetFactory_EntityPool_Field), new CodeMatch(OpCodes.Ldarg_0),
                new CodeMatch(OpCodes.Ldfld, UIBeltBuildTip_OutputEntityId_Field), new CodeMatch(OpCodes.Ldelem), new CodeMatch(OpCodes.Stloc_1));

            matcher.Advance(1).CreateLabel(out Label label);

            matcher.InsertAndAdvance(new CodeInstruction(OpCodes.Ldloc_1), new CodeInstruction(OpCodes.Ldfld, EntityData_AssemblerId_Field),
                new CodeInstruction(OpCodes.Ldc_I4_0), new CodeInstruction(OpCodes.Ble, label), new CodeInstruction(OpCodes.Ldloc_0), new CodeInstruction(OpCodes.Ldloc_1),
                new CodeInstruction(OpCodes.Ldarg_0), new CodeInstruction(OpCodes.Ldfld, UIBeltBuildTip_OutputEntityId_Field), new CodeInstruction(OpCodes.Ldarg_0),
                new CodeInstruction(OpCodes.Ldfld, UIBeltBuildTip_OutputSlotId_Field), new CodeInstruction(OpCodes.Ldarg_0),
                new CodeInstruction(OpCodes.Ldfld, UIBeltBuildTip_SelectedIndex_Field), new CodeInstruction(OpCodes.Call, MegaAssembler_SetFilterToEntity_Patch_Method));
        ...
        [HarmonyTranspiler]
        public static IEnumerable<CodeInstruction> UISlotPicker_SetFilterToEntity_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            var matcher = new CodeMatcher(instructions, generator);

            matcher.MatchForward(true, new CodeMatch(OpCodes.Ldloc_0), new CodeMatch(OpCodes.Ldfld, PlanetFactory_EntityPool_Field), new CodeMatch(OpCodes.Ldarg_0),
                new CodeMatch(OpCodes.Ldfld, UISlotPicker_OutputEntityId_Field), new CodeMatch(OpCodes.Ldelem), new CodeMatch(OpCodes.Stloc_1));

            matcher.Advance(1).CreateLabel(out Label label);

            matcher.InsertAndAdvance(new CodeInstruction(OpCodes.Ldloc_1), new CodeInstruction(OpCodes.Ldfld, EntityData_AssemblerId_Field),
                new CodeInstruction(OpCodes.Ldc_I4_0), new CodeInstruction(OpCodes.Ble, label), new CodeInstruction(OpCodes.Ldloc_0), new CodeInstruction(OpCodes.Ldloc_1),
                new CodeInstruction(OpCodes.Ldarg_0), new CodeInstruction(OpCodes.Ldfld, UISlotPicker_OutputEntityId_Field), new CodeInstruction(OpCodes.Ldarg_0),
                new CodeInstruction(OpCodes.Ldfld, UISlotPicker_OutputSlotId_Field), new CodeInstruction(OpCodes.Ldarg_0),
                new CodeInstruction(OpCodes.Ldfld, UISlotPicker_SelectedIndex_Field), new CodeInstruction(OpCodes.Call, MegaAssembler_SetFilterToEntity_Patch_Method));

            return matcher.InstructionEnumeration();
        }

        public static void SetFilterToEntity_Patch(PlanetFactory factory, EntityData entityData, int outputEntityId, int outputSlotId, int selectedIndex)
        {
            AssemblerComponent assemblerComponent = factory.factorySystem.assemblerPool[entityData.assemblerId];
        ...
        private static bool IsChangeCached((int, int) id, int slotId, int selectedIndex) => LastSetId == id && LastSlotId == slotId && LastSelectedIndex == selectedIndex;
