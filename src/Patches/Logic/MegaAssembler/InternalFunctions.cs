...
namespace ProjectGenesis.Patches.Logic.MegaAssembler
{
    internal static partial class MegaAssemblerPatches
    {
        ...
        internal static bool ContainsRecipeTypeRevert(ERecipeType recipetype, ERecipeType filter) =>
            ContainsRecipeType(filter, recipetype);
        ...
        internal static bool ContainsRecipeType(ERecipeType filter, ERecipeType recipetype)
        {
            ...
            switch ((Utils.ERecipeType)filter)
            {
                case Utils.ERecipeType.所有化工:
                    return type == Utils.ERecipeType.Chemical || type == Utils.ERecipeType.Refine ||
                           type == Utils.ERecipeType.高分子化工;
                ...
                case Utils.ERecipeType.所有制造:
                    return
                        type == Utils.ERecipeType.Chemical ||
                        type == Utils.ERecipeType.Refine ||
                        type == Utils.ERecipeType.Assemble ||
                        type == Utils.ERecipeType.Particle ||
                        type == Utils.ERecipeType.标准制造 ||
                        type == Utils.ERecipeType.高精度加工 ||
                        type == Utils.ERecipeType.Research ||
                        type == Utils.ERecipeType.高分子化工;
